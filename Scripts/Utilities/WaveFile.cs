...
namespace IBM.Watson.DeveloperCloud.Utilities
{
    ...
    static public class WaveFile
    {
        ...
        public static AudioClip ParseWAV(string clipName, byte[] data)
        {
            ...
            {
				Log.Error("WaveFile", "Malformed WAV header: {0} != RIFF || {1} != WAVE", GetID(form.form_id), GetID(form.id));
                return null;
            }
            ...
            while (reader.BaseStream.Position < reader.BaseStream.Length)
            {
                ...
                else if (GetID(chunk.id) == "data")
                {
                    if (!bHeaderFound)
                    {
						Log.Error("WaveFile", "Failed to find header.");
                        return null;
                    }
					...
					Log.Debug("WaveFile", "WAV INFO, channels = {0}, bps = {1}, samples = {2}, rate = {3}",
                    ...
                    {
						Log.Error("WaveFile", "Unspported BPS {0} in WAV data.", bps.ToString());
                        return null;
                    }
        ...
        public static byte[] CreateWAV(AudioClip clip, int bps = 16)
        {
            ...
            {
				Log.Error("WaveFile", "Unsupported BPS {0} in WAV data.", bps.ToString());
                return null;
            }
