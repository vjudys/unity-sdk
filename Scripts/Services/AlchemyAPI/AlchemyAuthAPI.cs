...
namespace IBM.Watson.DeveloperCloud.Services.AlchemyAuthAPI.v1
{
	/// <summary>
	/// Service integration for Alchemy API
	/// This implemenentation is using basic authentication, which Alchemy normally don't use.
	/// It is done to "fool" the security system that requires all API to have authentication.
	/// </summary>
    public class AlchemyAuthAPI : IWatsonService
    {

        #region Private Data
        private const string SERVICE_ID = "AlchemyAuthAPIV1";
        ...
        private void OnGetEntityExtractionResponse(RESTConnector.Request req, RESTConnector.Response resp)
        {
            ...
            if (resp.Success)
            {
                ...
                {
                    Log.Error("AlchemyAuthAPI", "OnGetEntityExtractionResponse Exception: {0}", e.ToString());
                    resp.Success = false;
                }
            }
        ...
        private void OnGetKeywoardExtractionResponse(RESTConnector.Request req, RESTConnector.Response resp)
        {
            ...
            if (resp.Success)
            {
                ...
                {
                    Log.Error("AlchemyAuthAPI", "OnGetKeywoardExtractionResponse Exception: {0}", e.ToString());
                    resp.Success = false;
                }
            }
        ...
        private void OnGetCombinedCallResponse(RESTConnector.Request req, RESTConnector.Response resp)
        {
            ...
            if (resp.Success)
            {
                ...
                {
                    Log.Error("AlchemyAuthAPI", "OnGetCombinedCallResponse Exception: {0}", e.ToString());
                    resp.Success = false;
                }
            }
        ...
        {
            private AlchemyAuthAPI m_Service = null;
            private ServiceStatus m_Callback = null;

            public CheckServiceStatus(AlchemyAuthAPI service, ServiceStatus callback)
            {
                m_Service = service;
                m_Callback = callback;
